        ...
        case 'vendor': {
            ...
            if (!password) {
                ...
                dbConnect(params, () => {
                    ...
                    vendor.checkVendor(file, password)
                        .then(() => {
            ...
            objects.getObject('system.config', (_err, systemConfig) => {
                objects.getObject('system.repositories', (err, repos) => {
                    ...
                    if (!err && repos && repos.native && repos.native.repositories) {
                        let active = systemConfig.common.activeRepo;
                        ...
                        for (let r = 0; r < active.length; r++) {
                            ...
                            if (repos.native.repositories[repo]) {
                                ...
                                if (!repos.native.repositories[repo].json) {
                                    console.log(`Update repository "${repo}" under "${repos.native.repositories[repo].link}"`);
                                    // Load it
                                    promises.push(new Promise(resolve =>
                                        (rep => {
                                            tools.getRepositoryFile(repos.native.repositories[rep].link, {
                                                ...
                                                if (_err && !sources) {
                                                    console.error(`Cannot get repository ${repos.native.repositories[rep].link}: ${_err}`);
                                                } else {
                        ...
                        if (!anyFound) {
                            console.log(`Requested repository "${active.join(', ')}" does not exit in config.`);
                            return void callback(EXIT_CODES.INVALID_REPO);
                        } else {
                            if (promises.length) {
